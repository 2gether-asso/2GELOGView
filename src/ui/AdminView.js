    ...
    return `
        <div class="glass-panel rounded-2xl p-5 space-y-4">
            <div class="flex items-center justify-between flex-wrap gap-2">
                <h3 class="text-lg font-black text-white">📅 Rétrospective ${year}</h3>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div class="bg-black/20 p-3 rounded-xl border border-white/5">
                    <div class="text-[10px] text-slate-500 uppercase font-bold">🔮 Prévus</div>
...
function renderAnomaliesSection(anomalies) {
    if (!anomalies || anomalies.length === 0) {
        return `
            <div class="glass-panel rounded-2xl p-5">
                <h3 class="text-sm font-black text-emerald-400 mb-1">✅ Aucune anomalie détectée</h3>
    ...
    const rows = anomalies.map(a => `
        <div class="flex items-start gap-2 px-3 py-2 rounded-lg ${a.severity === 'error' ? 'bg-rose-500/10 border border-rose-500/20' : 'bg-amber-500/10 border border-amber-500/20'}">
            <span class="text-xs font-black shrink-0 ${a.severity === 'error' ? 'text-rose-400' : 'text-amber-400'}">${a.severity === 'error' ? '⛔' : '⚠️'}</span>
            <div class="min-w-0">
    ...
    return `
        <div class="glass-panel rounded-2xl p-5 space-y-2">
            <h3 class="text-sm font-black text-white">🩺 ${anomalies.length} anomalie${anomalies.length > 1 ? 's' : ''} détectée${anomalies.length > 1 ? 's' : ''} dans le tableur</h3>
